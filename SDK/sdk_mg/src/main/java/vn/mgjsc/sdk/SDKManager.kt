...
import vn.mgjsc.sdk.api.PaymentApi
...
//        }
    ...
    fun getBannerCallback():BannerCallback? {
                ...
                //if (AccountManager.getInstance().configs != null && AccountManager.getInstance().configs!!.adsMigame!=null) {
                    ...
                    //if(AccountManager.getInstance().configs!!.adsMigame != null && AccountManager.getInstance().configs!!.adsMigame!!.size>0)
                        if(baseConfigModel!!.bannerMigame != null && baseConfigModel!!.bannerMigame!!.size > 0) {
                            val intent: Intent = Intent(activity, BannerMiGameActivity::class.java)
        ...
        public fun removeSkipBanner(context: Context?,listBanner : ArrayList<BannerMiGame>):ArrayList<BannerMiGame> {
            ...
            var jsonBanner = SDKManager.getBannerSkip(context!!)
...
//                showLoading()
                ...
                val clientOS = Constants.CLIENT_OS
                val clientIP = Device.getIPAddress(true)
                isTracking = true
                var sign = Encrypt.getHashString("${deviceId}${clientOS}${dateTime}",SDKManager.getSECRET_KEY())
        ...
        fun initSDK(application : Application, appKey : String? , secretKey : String? ,ggKey : String?, affKey : String?, adjustKey: String?) {
            ...
            //if()
            Constants.trustAllCertificates()
            // PQT initalize firebase
            FirebaseApp.initializeApp(application.applicationContext)
            // init FB
            FacebookSdk.InitializeCallback {
                Constants.showDataLog(Constants.LOG_TAG, "Init facebook ok!!!")
            }
            FacebookSdk.setAutoInitEnabled(true)
            FacebookSdk.setAutoLogAppEventsEnabled(true)
            // init tracking
            TrackingManager.init(application.applicationContext)
            registerEndPoint()
            INITSDK = true

            // reset payment trigger
            val sharedPreference = application!!.getSharedPreferences(SDKManager.PREFERENCE_ACCOUNT_MANAGER, Context.MODE_PRIVATE)
        ...
        fun initAdjustSDK(context:Context)
        {
          //  if(!TextUtils.isEmpty(key))
            ...
            if(baseConfigModel == null || baseConfigModel!!.adjustConfig == null) {
                ...
                try {
                    var assetManager = context.assets.open(Constants.ADJUST_KEY_CONFIG);
                ...
                if(isAdjustValid) {
                    ...
                    Constants.showDataLog(Constants.LOG_TAG,"missing Adjust Key Config file");
            ...
            }else
            {
               // Toast.makeText(context,"Missing adjust key param from SDK int",2000);
                isAdjustValid = false;
                Constants.showDataLog(Constants.LOG_TAG,"missing adjust key param from SDK init");
            }
            if(!isAdjustValid)
            {
...
//        }
        ...
        fun logout(context : Context, authUserCallback: AuthUserInterface, showLogin: Boolean =false) {
            this.authUserCallback = authUserCallback
            var intent = Intent(context , MGActivity::class.java)
            var temp = Constants.ACTION_LOGOUT;
            if(!showLogin)
                temp = Constants.ACTION_LOGOUT_ONLY;
            intent.putExtra(
                Constants.KEY_ACTION,
                temp)
            context.startActivity(intent)
        ...
        fun payment(context : Context,packageId : String,orderID:String,serverID:String,roleID:String?="",roleName:String?="",otherData:String?="", paymentUserCallback: PaymentUserCallback) {
            ...
            val sharedPreference = context!!.getSharedPreferences(SDKManager.PREFERENCE_ACCOUNT_MANAGER, Context.MODE_PRIVATE)
            var lastTimePayment = sharedPreference.getLong(KEY_SHARED_PREFERENCES_LAST_PAYMENT_ACTION,0);
            Constants.showDataLog("MG_SDK","enter payment");
            if(SystemClock.elapsedRealtime() - lastTimePayment < 2000 )
            {
                Constants.showDataLog("MG_SDK","skip payment due to 2 clicks too nearly");
                return;
            }
            ...
            intent.putExtra(
                Constants.KEY_DATA_PACKAGE_ID,
                packageId)
            intent.putExtra(
                Constants.KEY_DATA_ORDER_ID,
                orderID)
            intent.putExtra(
                Constants.KEY_DATA_SERVER_ID,
                serverID)
            intent.putExtra(
                Constants.KEY_DATA_ROLE_ID,
                roleID)
            intent.putExtra(
                Constants.KEY_DATA_ROLE_NAME,
                roleName)
            intent.putExtra(
                Constants.KEY_DATA_OTHER_DATA,
                otherData)
        ...
        fun setIsDebug(isDebug:Boolean)
        {
            ...
            if(isDebug)
            {
                Constants.showDataLog(Constants.LOG_TAG, getAPP_KEY())
                Constants.showDataLog(Constants.LOG_TAG, getSECRET_KEY())
                if(APPLICATION != null) {
                    Constants.showDataLog(
                        Constants.LOG_TAG,
                    ...
                    Constants.showDataLog(
                        Constants.LOG_TAG, FacebookSdk.getApplicationSignature(
                            APPLICATION
                        )
                    )
                    Constants.getHashKey(APPLICATION)
                    Constants.getSHA(APPLICATION,"SHA1")
                    Constants.getSHA(APPLICATION,"SHA256")
                }
            }
        }
