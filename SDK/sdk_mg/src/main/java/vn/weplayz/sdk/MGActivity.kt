...
import vn.weplayz.sdk.databinding.ActivityMgBinding
import vn.weplayz.sdk.databinding.MgFragmentForgetPassBinding
import vn.weplayz.sdk.databinding.MgFragmentLoginBinding
import vn.weplayz.sdk.databinding.MgFragmentRegisterBinding
import vn.weplayz.sdk.databinding.MgFragmentSyncAccountBinding
import vn.weplayz.sdk.databinding.MgMainAccountBinding
...
//        activateImmersiveMode ImmersiveControl.(this);
        ...
        binding.debugMode.setOnClickListener {
           // Log.d("pqt", "test ----")
            if(ValidateSDK.isDebugModeOn()) {
                SDKWeplayZManager.setIsDebug(true)
    ...
    {
        vn.weplayz.sdk.constants.Constants.showDataLog("packageID",packageID)
        vn.weplayz.sdk.constants.Constants.showDataLog("orderID",orderID)
        vn.weplayz.sdk.constants.Constants.showDataLog("roleID",roleID)
        vn.weplayz.sdk.constants.Constants.showDataLog("roleName",roleName)
        vn.weplayz.sdk.constants.Constants.showDataLog("otherData",otherData)
        vn.weplayz.sdk.constants.Constants.showDataLog("accessToken",accessToken)
...
//        setIntent(intent)
        if(stateProcess == vn.weplayz.sdk.constants.Constants.ACTION_PAYMENT)
            if(TextUtils.isEmpty(packageID) || TextUtils.isEmpty(orderID) || TextUtils.isEmpty(accessToken)) {
            packageID = intent?.getStringExtra(vn.weplayz.sdk.constants.Constants.KEY_DATA_PACKAGE_ID)
            serverID = intent?.getStringExtra(vn.weplayz.sdk.constants.Constants.KEY_DATA_SERVER_ID)
            orderID = intent?.getStringExtra(vn.weplayz.sdk.constants.Constants.KEY_DATA_ORDER_ID)
            roleID = intent?.getStringExtra(vn.weplayz.sdk.constants.Constants.KEY_DATA_ROLE_ID)
            roleName = intent?.getStringExtra(vn.weplayz.sdk.constants.Constants.KEY_DATA_ROLE_NAME)
            otherData = intent?.getStringExtra(vn.weplayz.sdk.constants.Constants.KEY_DATA_OTHER_DATA)
...
//                paramsIAP?.accessToken = token
    ...
    val runnable = Runnable { SDKWeplayZManager.trackerIAP(this) }
    val handle = Handler()
    fun initSDK()
    {
        ...
        sharedPreferences = getSharedPreferences(SDKWeplayZManager.PREFERENCE_ACCOUNT_MANAGER, Context.MODE_PRIVATE)
        ...
        if(intent != null)
        {
            val temp = intent.getStringExtra(vn.weplayz.sdk.constants.Constants.KEY_ACTION)
            if(!TextUtils.isEmpty(temp)) {
                stateProcess = temp!!

                if(stateProcess == vn.weplayz.sdk.constants.Constants.ACTION_LOGOUT_ONLY)
                    hideMainMenu = true;
            }
        ...
        when(stateProcess)
        {
            ...
            vn.weplayz.sdk.constants.Constants.ACTION_LOGOUT->{
                processLogout()
            }
            vn.weplayz.sdk.constants.Constants.ACTION_LOGOUT_ONLY->{
                processLogoutOnly()
            }
    ...
    private fun registerMainMenuScreen()
    {
        ...
        bindingMainAccount.mgMainAccountTvLogin.setOnClickListener {
            ...
            if(stepDebug>6)
                SDKWeplayZManager.setIsDebug(true)
            if(stepDebug>12)
                SDKWeplayZManager.setIsDebug(false)
        }
    }
...
//        }
        if(result)
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_login_validate_success),json)
        else
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_login_validate_failed),json)
    ...
    private fun registerRegisterScreen()
    {
        ...
        bindingFgRegister.mgFragmentRegisterTvRegister.setOnClickListener {
            ...
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_click_register),null)
        }
  ...
  //      }
    ...
    private fun updateViewSync()
    {
        ...
        SDKWeplayZManager.baseConfigModel?.let {
            ...
            str = str.replace(Regex("_URL_"),SDKWeplayZManager.baseConfigModel!!.urlPolicy)
        ...
        var isShow : Int = 0
        SDKWeplayZManager.baseConfigModel?.let {
            isShow = SDKWeplayZManager.baseConfigModel!!.isRequireMoreInfo
...
//                    bindingFgRegister.mgFragmentRegisterEdDob.setPadding(bindingFgRegister.mgFragmentRegisterEdDob.paddingLeft,bindingFgRegister.mgFragmentRegisterEdDob.paddingTop,ValidateSDK.convertDpToPixel(50.0f,context!!).toInt(),bindingFgRegister.mgFragmentRegisterEdDob.paddingRight)
        ...
        if(result)
        {
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_register_validate_success),json)
        }else{
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_register_validate_failed),json)
        }
        return result
    }
...
//
//        }
        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_sync_validate),null)
        val json = JSONObject()
...
//                    bindingFgSync.mgFragmentSyncEdDob.setPadding(bindingFgSync.mgFragmentSyncEdDob.paddingLeft,bindingFgSync.mgFragmentSyncEdDob.paddingTop,ValidateSDK.convertDpToPixel(50.0f,context!!).toInt(),bindingFgSync.mgFragmentSyncEdDob.paddingRight)
    ...
    override  fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
        ...
        vn.weplayz.sdk.utils.ImmersiveControl.onKeyDown(this,keyCode);
    }
...
//        super.onBackPressed()
    ...
    var stateProcess : String  = vn.weplayz.sdk.constants.Constants.ACTION_LOGIN
    private fun onCLickLoginFacebook()
    {
        ...
        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_click_login_fb),null)
    ...
    private fun onCLickLoginGoogle() {
        ...
        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_click_login_gg),null)
    ...
    private fun nextState(state:String)
    {
        if(!currentState.equals(STATE_DONE))
            return
        nextState = state
        if(SDKWeplayZManager.baseConfigModel == null)
        {
            getConfig()
        }
        else
...
//                    showToast(getString(R.string.mg_text_login_fb_failed))
                             ...
                             hideLoading()
                             vn.weplayz.sdk.constants.Constants.showDataLog("Login FB",": ${exception.message}")
                             }
                         });
            ...
            SDKWeplayZManager.baseConfigModel?.let {
                ...
                SynQuickDeviceApi.getSynQuickDevice(
                        ...
                        sign = Encrypt.getHashCodeSyncAccount(
                            ...
                            deviceId,
                            dateTime,
                            SDKWeplayZManager.getSECRET_KEY()
                        )
                    ) { user, e ->
                        hideLoading()
                        currentState = STATE_DONE
...
//        System.out.println("songsong::"+MiGameSDK.getAppGoogleKey())
        ...
        gso = GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
            .requestIdToken(SDKWeplayZManager.getGG_KEY())
            .requestEmail()
            .build()
    ...
    {
        val clientKey = SDKWeplayZManager.getTIKTOK_KEY();
        val scope = "user.info.basic"
        val state = UUID.randomUUID().toString()
        val redirectUri = SDKWeplayZManager.getTIKTOK_REDIRECT_URL()
            ...
            SDKWeplayZManager.baseConfigModel?.let {
                ...
                if (NetworkUtils.isNetworkConnected(context!!)) {
                    ...
                    AuthenApi.getLoginTiktok(
                        URL_TiktokLogin,
                        SDKWeplayZManager.getTIKTOK_KEY(),
                        SDKWeplayZManager.getTIKTOK_CLIENT_KEY(),
                        authCode,
                        codeVerifier!!,
                        SDKWeplayZManager.getTIKTOK_REDIRECT_URL(),
                        deviceID,
                        dateTime,
                        Encrypt.getHashCodeLoginTiktok(authCode,SDKWeplayZManager.getTIKTOK_KEY(),SDKWeplayZManager.getTIKTOK_CLIENT_KEY(),dateTime,SDKWeplayZManager.getSECRET_KEY())
                    )
                    { user, e ->
                        hideLoading()
                        currentState = STATE_DONE
                        ...
                        if (user != null) {
                            ...
                            loginSuccess(user)
                            vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(
        ...
        SDKWeplayZManager.baseConfigModel?.let {
            if(NetworkUtils.isNetworkConnected(context!!)) {
                ...
                AuthenApi.getLoginFacebook(
                    ...
                    if (user != null) {
                        loginSuccess(user)
                        vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_fb_success), null,null)
...
//                        loginEventFailed(getString(R.string.migamesdk_event_login_fb_failed),e?.message)
                        vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_fb_failed),null,e?.message)
                    }
                }
            }else
            {
...
//        completedTask: Task<GoogleSignInAccount>
        ...
        try {
            ...
            e.printStackTrace()
            hideLoading()
            vn.weplayz.sdk.constants.Constants.showDataLog(vn.weplayz.sdk.constants.Constants.LOG_TAG, "signInResult:failed code=${e.statusCode}: ${e.message}")
        }
    }
...
//                            loginEventSuccess(getString(R.string.migamesdk_event_login_gg_success))
                            vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_gg_success),null,null)
                        } else {
//                            authListener?.onLoginFail(e?.message)
//                            loginEventFailed(
...
//                                e?.message
//                            )
                            vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_gg_failed),null,e?.message)
                            loginFailed(e?.message)
                        }
                    }
                } else {
...
//                        loginEventSuccess(getString(R.string.migamesdk_event_login_qp_success))
                        vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_qp_success),null,null)
                    } else {
                        showMessageError(e?.message)
                        loginFailed(e?.message)
//                        authListener?.onLoginFail(e?.message)
//                        loginEventFailed(getString(R.string.migamesdk_event_login_gg_failed),e?.message)
                        vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_qp_failed),null,e?.message)
                    }
                }
            }
            else
    ...
    private fun synSkip()
    {
        if(getUser()!=null) {
            ...
            if(checkForceUserSynAccount(getUser()))
            {
                ...
                SDKWeplayZManager.authUserCallback?.onUserLoginSuccess(getUser()!!)
                SDKWeplayZManager.authUserCallback?.onUserSyncCancel(getUser()!!)
                closeActivity()
            }
        }
        else
    ...
    {
        saveUser(user)
        SDKWeplayZManager.authUserCallback?.onUserLoginSuccess(user)
        SDKWeplayZManager.authUserCallback?.onUserSyncSuccess(user)
        closeActivity()
    }
...
//        SDKManager.authUserCallback?.onUserLoginSuccess(user)
    ...
    {
        stateProcess = vn.weplayz.sdk.constants.Constants.ACTION_LOGIN
        saveUser(null)
//        closeActivity()
        SDKWeplayZManager.authUserCallback?.onUserLogoutSuccess()
        if(hideMainMenu )
            closeActivity()
    }
    private fun logoutFailed(accessToken:String,e:String?)
    {
        stateProcess = vn.weplayz.sdk.constants.Constants.ACTION_LOGIN
//        closeActivity()
        SDKWeplayZManager.authUserCallback?.onUserLogoutFail(e)
        if(hideMainMenu)
            closeActivity()
    }
    ...
    {
        showMessageError(e)
        SDKWeplayZManager.authUserCallback?.onUserLoginFail(e)
    }
    ...
    private fun getGender() : String
    {
        ...
        if(bindingFgRegister.mgFragmentRegisterRdoMale.isChecked)
            gender = vn.weplayz.sdk.constants.Constants.GENDER_MALE
        else
            if(bindingFgRegister.mgFragmentRegisterRdoMale.isChecked)
                gender = vn.weplayz.sdk.constants.Constants.GENDER_FEMALE
            else
                if(bindingFgRegister.mgFragmentRegisterRdoMale.isChecked)
                    gender = vn.weplayz.sdk.constants.Constants.GENDER_OTHER
        return gender
    }
                ...
                SDKWeplayZManager.baseConfigModel?.let {
                    ...
                    AuthenApi.getRegister(
                        ...
                        if (user != null) {
                            ...
                            json.put("username",user.userName)
                            json.put("userid",user.userId)
                            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_register_success),json)
                        } else {
                            loginFailed(e?.message)
//                            showToast(e?.message)
...
//                            hashMapOf(fragment_login_ed_account.text.toString() to "username"))
                        vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_success),
...
//                        loginEventFailed(getString(R.string.migamesdk_event_login_failed),e?.message,hashMapOf(fragment_login_ed_account.text.toString() to "username"))
                        vn.weplayz.sdk.utils.TrackingManager.trackLoginEvent(context?._getString(R.string.mg_event_login_failed),hashMapOf("username" to bindingFgLogin.mgFragmentLoginEdAccount.text.toString()),e?.message)
                        if (countLogin > 2)
                            showCaptcha()
                    }
                }
	...
	public fun checkAndShowBanner(obj: Any)
    {
        ...
        if(obj is BaseConfigsDataModel){
            ...
            if(!ValidateSDK.isStringNullOrEmpty(config.linkWeb) || !ValidateSDK.isStringNullOrEmpty(config.dataWeb)){
                val intent: Intent = Intent(this, vn.weplayz.sdk.BannerMiGameActivity::class.java)
    ...
    private fun loadConfigSuccess(config:BaseConfigsDataModel)
    {
        handle.removeCallbacks(runnable)
        handle.post(runnable)
        SDKWeplayZManager.baseConfigModel = config
        updateButtonAction()
        updateViewRegister()
        updateViewSync()
    ...
    private fun updateButtonAction()
    {
        ...
        SDKWeplayZManager.baseConfigModel?.SDKShowConfig?.let {
            isShowAccount = SDKWeplayZManager.baseConfigModel!!.SDKShowConfig!!.isShowAccount
            isShowQuickPlay = SDKWeplayZManager.baseConfigModel!!.SDKShowConfig!!.isShowQuickPlay
            isShowFB = SDKWeplayZManager.baseConfigModel!!.SDKShowConfig!!.isShowFB
            isShowGG = SDKWeplayZManager.baseConfigModel!!.SDKShowConfig!!.isShowGG
...
//            return
        ...
        SDKWeplayZManager.baseConfigModel?.let { config ->
            if (NetworkUtils.isNetworkConnected(context)) {
                ...
                iAPManager = IAPSDKManager(this@MGActivity, config.extraValue.IAP_Android_PublicKey, object : IAPSDKManager.EventIAPManager {
                    override fun onPaymentReady() {
                        ...
                        if(isCreateTrans==false && transactionIAP==null)
                            getCreateTransStore()
                        vn.weplayz.sdk.constants.Constants.showDataLog("iap game","ready")
                    }
                    ...
                    override fun onPaymentSuccess(purchase: Purchase) {
                        ...
                        if(transactionIAP != null) {
                           ...
                           vn.weplayz.sdk.utils.TrackingManager.trackEventRevenue(
                             context?._getString(R.string.mg_event_revenue_iap),
                           valueVND,
                         js.toString()
                        )
                        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_payment_success_iap),js)
                        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_payment_total_payment),js)
                        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_payment_total_complete_success),js)
                        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?.getString(R.string.mg_event_payment_success),js)
...
//            callAPIGetConfigData(typeGetConfig = TYPE_GET_CONFIG_INIT_IAP)
    ...
    private fun getChargeToGame(
        ...
        vn.weplayz.sdk.constants.Constants.showDataLog(vn.weplayz.sdk.constants.Constants.LOG_TAG,"getChargeToGame")
...
//        val purchaseToken = purchase.responseData
        SDKWeplayZManager.baseConfigModel?.let {
            ...
            val dateTime = SDKParams.getCurrentTime()
             PaymentApi.getChargeToGame(
                ...
                roleID.getValue(),
                roleName.getValue(),
                vn.weplayz.sdk.constants.Constants.CLIENT_OS,
                deviceId,
                otherData.getValue(),
                useCoin,
                ...
                Encrypt.getHashCodeGetChargeToGame(
                    ...
                    orderIDIAP,
                    packageStoreID.getValue(),
                    dateTime,SDKWeplayZManager.getSECRET_KEY()
                )
            ) { result, e ->
                if (result != null) {
    ...
    private fun onPaymentSuccess(chargeToGameResult: PaymentDataGameModel?) {
        closeActivity()
        SDKWeplayZManager?.paymentUserCallback?.onUserPaymentSuccess(chargeToGameResult)
     ...
     //   Log.d("PQT Debug","transaction " + transactionIAP!!.transactionID + " AMOUNT " + transactionIAP!!.amount)
        ...
        if(isUseCoin) {
            ...
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_payment_success_micoin),js)
            vn.weplayz.sdk.utils.TrackingManager.trackEventRevenue(context?._getString(R.string.mg_event_revenue_total), valueVND , js.toString())
    ...
    private fun onPaymentFail(e: String?){
        ...
        this.showMessage {
            ...
            listener = object : AppMessage.EventMessage {
                override fun onClickConfirm() {
                    super.onClickConfirm()
                    SDKWeplayZManager?.paymentUserCallback?.onUserPaymentFail(e)
                    closeActivity()
        ...
        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?.getString(R.string.mg_event_payment_total_complete_failed),js)
        if(isUseCoin)
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_payment_failed_micoin),js);
        else
            vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_payment_failed_iap),js);
...
//            }
            ...
            if(SDKWeplayZManager.baseConfigModel!!.disablePopupCoin == 0) {
                showMessageConfirm {
                    title = getString(R.string.mg_text_message_title)
...
//        AccessToken + Package + DeviceID + ClientOS + StoreProductID + Time , SecretKey)
        ...
        val runnable : Runnable = Runnable {
            SDKWeplayZManager.baseConfigModel?.let {
                ...
                val dateTime = SDKParams.getCurrentTime()
                val deviceId = Device.getDeviceID(this@MGActivity)
                val data = "${accessToken.getValue()}${packageID.getValue()}${deviceId.getValue()}${vn.weplayz.sdk.constants.Constants.CLIENT_OS.getValue()}${dateTime.getValue()}"
                PaymentApi.getCreateTrans(
                    ...
                    Encrypt.getHashString(
                        data,SDKWeplayZManager.getSECRET_KEY())
                ) { result, e ->
                    if (result != null) {
        ...
        context?.let {
            if(NetworkUtils.isNetworkConnected(context!!)) {
                ...
                ConfigApi.getConfig(
                    deviceID,
                    dateTime,
                    Encrypt.getHashCodeConfig(DeviceID = deviceID,RequestTime = dateTime,appSecretKey = SDKWeplayZManager.getSECRET_KEY())
                ) { config, e ->
                    hideLoading()
                    ...
                    if (config != null) {
                      ...
                      //  if(SDKManager.baseConfigModel.adjustConfig == null)
                       ...
                       // Log.d("PQT Debug","hhhhhhhhhh" + SDKManager.baseConfigModel!!.urlIAPCreateTrans);
                        ...
                        SDKWeplayZManager.baseConfigModel?.let {
                         SDKWeplayZManager.adjustConfig = SDKWeplayZManager.baseConfigModel!!.adjustConfig;
                        }
...
//                        Log.d("PQT Debug-----","config::" + config.toString())
  //                      Log.d("PQT Debug-----","Adjust config::" + SDKManager.adjustConfig.toString())
                        SDKWeplayZManager.initAdjustSDK(applicationContext)
...
//                    }
    ...
    private fun verifyAccessToken(accessToken: String) {
        ...
        vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_verify_token),null)
        context?.let {
            ...
            SDKWeplayZManager.baseConfigModel?.let {
                ...
                showLoading()
                    val deviceId = Device.getDeviceID(context!!)
                    UserApi.getUserByAccessToken(accessToken, deviceId, payment, SDKWeplayZManager.baseConfigModel!!.User_GetByToken,dateTime,
                        Encrypt.getHashCodeGetUser(accessToken, dateTime, SDKWeplayZManager.getSECRET_KEY()),
                        { user, e ->
                        currentState = STATE_DONE
                        hideLoading()
                        if (user != null) {
                            ...
                            nextState();
                            loginSuccess(user)
                                vn.weplayz.sdk.utils.TrackingManager.trackEventCount( context?._getString(R.string.mg_event_verify_token_success), null)
                        } else {
                            val js = JSONObject()
                            js.put("error", e?.message)
                            verifyAccessTokenFailed(e?.message)
                                vn.weplayz.sdk.utils.TrackingManager.trackEventCount(context?._getString(R.string.mg_event_verify_token_failed), js)
                        }
...
//    private val KEY_SHARED_PREFERENCES_GOOGLE = "vn.mgjsc.sdk.google"
    ...
    private var typePayment : Int = vn.weplayz.sdk.constants.Constants.TYPE_PAYMENT_DIRECT_PACKAGE
...
//        }
    ...
    fun saveInfoIAP(data: JSONArray?) {
        if(data!=null && sharedPreferences!= null ) {
            ...
            sharedPreferences!!.edit()!!.putString(SDKWeplayZManager.KEY_SHARED_PREFERENCES_INFO_IAP,dataStr).apply()
        }
        else
        {

            sharedPreferences!!.edit()!!.putString(SDKWeplayZManager.KEY_SHARED_PREFERENCES_INFO_IAP, JSONArray().toString()).apply()
        }
    }
    private fun getInfoIAP() : JSONArray
    {
        ...
        if(sharedPreferences!=null) {
            val data = sharedPreferences!!.getString(SDKWeplayZManager.KEY_SHARED_PREFERENCES_INFO_IAP, "")
            try {
   ...
   //     if (callbackManagerLoginFB!= null && callbackManagerLoginFB.onActivityResult(requestCode, resultCode, data)) {
    ...
    private fun removeInfoIAP(transactionID : String) {
        try {
            ...
            vn.weplayz.sdk.constants.Constants.showDataLog("save iAP", e.toString())
        }
    }
